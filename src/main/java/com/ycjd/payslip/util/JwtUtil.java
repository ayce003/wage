...
public class JwtUtil {
    ...
    public static String createJWT(String id, String subject,Map<String,Object> claims, long ttlMillis) {
        ...
        if(claims!=null){
            builder.setClaims(claims); //如果有私有声明，一定要先设置这个自己创建的私有的声明，这个是给builder的claim赋值，一旦写在标准的声明赋值之后，就是覆盖了那些标准的声明的
                ...
                .setSubject(subject)        //sub(Topics)：代表这个JWT的主体，即它的所有人，这个是一个json格式的字符串，可以存放什么userid，roldid之类的，作为什么用户的唯一标志。
    ...
    public static SecretKey generalKey(){
        ...
        //System.out.println(Base64Util.encodeBase64URLSafeString(encodedKey));//7786df7fc3a34e26a61c034d5ec8245d
        SecretKey key = new SecretKeySpec(encodedKey, 0, encodedKey.length, "AES");// 根据给定的字节数组使用AES加密算法构造一个密钥，使用 encodedKey中的始于且包含 0 到前 leng 个字节这是当然是所有。（后面的文章中马上回推出讲解Java加密和解密的一些算法）
        return key;
    }
    public static void main(String[] args) throws Exception {
        String ab=createJWT("jwt", "{id:100,name:xiaohong}",null, 6000000);
